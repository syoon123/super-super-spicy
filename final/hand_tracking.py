...
trackers = []
for i in range(6):
    trackers.append(dlib.correlation_tracker()) # trackers for key landmarks on face for pose estimation
...
#     glBegin(GL_QUADS)
#     glTexCoord2f(0.0, 0.0)
#     glVertex3f(-WIDTH * 0.003, HEIGHT * 0.003, 0.0)
#     glTexCoord2f(1.0, 0.0)
#     glVertex3f(WIDTH * 0.003, HEIGHT * 0.003, 0.0)
#     glTexCoord2f(1.0, 1.0)
#     glVertex3f(WIDTH * 0.003, -HEIGHT * 0.003, 0.0)
...
#     if (len(rects) > 0):
#         pos_available = True
#         tracker.start_track(image, rects[0])
...
#             for i in range(len(indices)):
#                 trackers[i].update(image)
#                 p = trackers[i].get_position()
#                 # cv2.rectangle(frame, (int(p.left()), int(p.top())), (int(p.right()), int(p.bottom())), (0, 255, 0), 2)
#                 shapes.append(((p.left() + p.right()) / 2., (p.top() + p.bottom()) / 2.))

#     if (pos_available):
#         r, t = estimate_pose(image, shapes)
#         z = t[2] * -750/(width * height)
#         glPushMatrix()
#         glTranslatef(0, 0, z)
#         rmtx = cv2.Rodrigues(r)[0]
#         view_matrix = np.array([[rmtx[0][0], rmtx[0][1], rmtx[0][2], 1.75*t[0]/width],
#                                 [rmtx[1][0], rmtx[1][1], rmtx[1][2], 1.75*t[1]/height],
#                                 [rmtx[2][0], rmtx[2][1], rmtx[2][2], 0],
#                                 [0.0, 0.0, 0.0, 1.0]])
#         view_matrix = view_matrix * INVERSE_MATRIX
#         view_matrix = np.transpose(view_matrix)
